...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `mode` | string | 否 | `"A"`（评估赛道型经理）或 `"B"`（轮动型行业推荐）。不填则自动从 `manager_name` / `target_industry` 推断 |
...
- **模式A（赛道型经理评估）**：给定基金经理，输出 Brinson 归因+产业时钟感评分，量化回答"TA是靠赛道 beta 还是 beta+alpha 双驱动"
- **模式B（轮动型经理推荐）**：给定产业方向，从内置档案库扫描8位经理，按持仓匹配度+历史产业超额排名，输出推荐列表
...
│   └── fund_top10_holdings.csv          # 前十大重仓股（2024Q4/2025Q4/2026Q1）
│
...
读取全部CSV，撰写 **1200-1500 字**的结构化报告，**优先用表格和数据说话，文字仅作关键节点注释**：

**报告结构**：
1. **一句话结论**（50字以内）——直接给出"赛道型/轮动型"判断 + 产业时钟感总评分（X/15分）
2. **Brinson 归因摘要表**（1个表，季度汇总）——行业配置贡献 vs 选股贡献，不超过200字说明
3. **产业时钟吻合度评分表**（1个表，含入场/加码/退出3维）——配1-2句关键节点文字注释，不展开叙述
4. **持仓×景气度匹配图**（Markdown表格模拟，关键季度×核心指标）——突出"提前/滞后/同步"
5. **能力标签**（3个词以内）+ **适合场景**（1句话）

**格式铁律**：全部使用Markdown表格；百分比精确到两位小数；禁止"约"、"大约"等模糊词；不使用emoji；**严控1200-1500字，超出则删减文字保留表格**。
...
**推荐报告**（`{target_industry}_推荐报告.md`，**400-600字**）结构：
1. 行业景气1句话定位（`clock_logic` + 最新数据，≤50字）
2. 评分排名表（完整CSV嵌入，无需额外文字重复内容）
3. Top3经理关键词（每人≤30字，只写最突出的1个特点）
4. 配置建议（1句话，含时点判断）

**格式铁律**：排名表是核心，文字是注释，不重复表中数据；禁止展开叙述；超出600字则删减文字。
